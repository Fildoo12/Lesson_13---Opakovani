...
public class TestUtils {
    ...
    public static List<String> getDatesWhenDrinkIsFruitDrinkOrSyrup(List<LunchItem> items) {
        return items.stream().filter(lunchItem -> lunchItem.getKind().equals(LunchItemKind.DRINK)).filter(lunchItem -> lunchItem.getName()
                .contains("Sirup") || lunchItem.getName().contains("Ovocný nápoj")).map(p -> String.format(p.getDate())).collect(Collectors.toList());
    }
